#### 第 1 套：初级（SpringBoot 入门 + HTTP 协议 + Tomcat）
##### 一、业务场景
开发一个“图书服务入门接口”，用于完成 SpringBoot 项目的快速搭建，并提供最基础的 HTTP 查询和新增接口。

##### 二、题目要求
1. 创建一个 SpringBoot Web 项目；
2. 在配置文件中完成以下配置：
  - 服务端口改为 8081
  - 访问路径前缀设置为 /api
3. 定义 BookController，完成以下接口：
  - GET /api/books/hello：返回字符串 "springboot ok"
  - GET /api/books/{id}：根据 id 返回一本图书信息
  - POST /api/books：接收图书名称、作者、价格，返回新增成功的信息
4. 要求接口返回结果能够通过 Apipost / Postman 正常访问；
5. 图书数据可先使用假数据模拟，不要求接数据库。

#### 第 2 套：中级（请求参数绑定 + 响应结果 + 相关注解）
##### 一、业务场景
开发一个“员工查询与新增接口”，要求后端能够接收不同类型的请求参数，并统一返回 JSON 结果。

##### 二、题目要求
1. 定义员工实体 Emp，字段至少包含：
  - id
  - name
  - gender
  - entryDate
  - deptId
2. 定义统一返回结果类 Result；
3. 编写 EmpController，完成以下接口：
  - GET /emps：根据姓名、部门 id 进行条件查询，其中参数为可选参数
  - GET /emps/{id}：根据路径参数查询员工
  - POST /emps：接收 JSON 格式的员工数据并新增
  - GET /emps/date：接收日期参数，要求正确绑定日期类型
4. 参数绑定要求：
  - 至少使用到 @RequestParam
  - 使用 @PathVariable
  - 使用 @RequestBody
  - 日期参数使用合适的日期格式注解处理
5. 所有接口统一返回 Result 对象。

#### 第 3 套：中高级（配置文件 + 多环境 + 全局异常）
##### 一、业务场景
开发一个“员工后台管理接口”，要求通过配置文件管理系统参数，并对异常进行统一处理，提升系统可维护性。

##### 二、题目要求
1. 在配置文件中定义以下自定义配置：
  - app.name
  - app.default-page-size
  - app.upload-path
2. 使用以下两种方式中的至少一种读取配置：
  - @Value
  - @ConfigurationProperties
3. 配置多环境文件，至少包含：
  - dev
  - test
4. 编写员工分页查询接口：
  - GET /emps/page
  - 接收页码、页大小参数
  - 当页大小不传时，使用配置文件中的默认页大小
5. 自定义业务异常类，例如：
  - 员工不存在异常
  - 参数非法异常
6. 使用全局异常处理器统一处理异常，并返回统一错误结果。

#### 第 4 套：高级（文件上传 + Filter + 部门管理综合）
##### 一、业务场景
开发一个“部门资料管理模块”，管理员可以维护部门信息，并上传部门展示图片。系统要求通过过滤器做简单登录校验。

##### 二、题目要求
1. 定义部门实体 Dept，字段至少包含：
  - id
  - name
  - createTime
  - updateTime
  - logo
2. 编写部门管理接口：
  - GET /depts：查询所有部门
  - POST /depts：新增部门
  - PUT /depts：修改部门
  - DELETE /depts/{id}：删除部门
3. 编写文件上传接口：
  - POST /upload
  - 接收 MultipartFile
  - 将文件保存到本地指定目录
  - 返回文件访问路径或保存结果
4. 编写过滤器 LoginCheckFilter：
  - 对 /depts/*、/upload 等接口进行拦截
  - /login 不拦截
  - 要求通过请求头中的 token 做简单校验
5. 文件保存路径从配置文件中读取。
